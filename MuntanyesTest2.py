...
for row in table.findAll("tr"):#per cada fila....
    for cell in row.findAll("td"):#busquem totes les cel.les
        if result == "": #si es el primer <td> l'afexeixo a resultat
            ...
            for a in cell.findAll('a', text=True):
                if i == 0:#si el el primer <a> li poso un punt i coma (per el csv)
                    ...
                    result = result + ", " + a.find(text=True)
            i=0 
        elif len(cell.findAll("sup")) != 0:#aquesta condicio hauria de controlar el <td> buit
            result = result + cell.find(text=True) #les a's son per veure on me les posa
